...
class Server:
    ...
    def __init__(self, address, port):
        ...
        # Init database
        self.database = database.Database("./database.sqlite")
        self.database.setup()
    ...
    def client_thread(self, client):
        ...
        if self.database.check_if_exist("users", 0, username) and self.database.check_row_column(self.database.get_user("users", username), 1, password) and commons.check_dict(self.onlineUsers, username, True) == False:
            ...
            client.send(json.dumps("Success"))
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        client.send(json.dumps(return_message))
                    except socket.error:
            ...
            client.send(json.dumps("Same user already logged in."))
            client.close()
        else:
            client.send(json.dumps("Incorrect username or password."))
            client.close()